...
const QuoteList = (props) => {
  ...
  const ChangeSortingHandler = () => {
    history.push("/quotes?sort=" + (isSortingAscending ? "desc" : "asc")); // location.push가 리랜더링 시킨다. 페이지를 push할 때, 지금 보고 있는 페이지일지라도 페이지 컴포넌트는 재평가된다.(react router는 history를 바꿨다고 봐서 페이지를 다시 렌더링하고, 그걸 띄운다.)
  };
